...
> 拿到一个业务需求，我会从四个维度来设计测试用例，确保覆盖完整且不冗余：
>
> **第一维度：业务场景法 —— 覆盖核心流程**
>
> 先把业务端到端的主路径走一遍。比如在 Gate.io 测余币宝申购，核心场景就是：
> 用户选择产品 → 输入金额 → 确认申购 → 资金扣款 → 资产确认 → 起息。
> 每条完整链路都变成一个场景用例。这样保证"业务能跑通"。
>
> **第二维度：等价类 + 边界值 —— 覆盖输入空间**
>
> 对每个输入参数做等价类划分。比如申购金额：
> - 有效等价类：100 ≤ 金额 ≤ 1,000,000 的正常区间
> - 无效等价类：金额 < 100（小于起投额）、金额 > 1,000,000（超过限额）
> - 边界值：99、100、101、999,999、1,000,000、1,000,001
> - 特殊值：0、负数、小数（如果只支持整数）
>
> **第三维度：异常和容错 —— 覆盖非正常路径**
>
> 这是最容易漏的部分。比如：网络超时是否自动重试？余额不足是否明确提示？重复提交是否被幂等拦截？并发申购是否出现超仓？状态冲突（已赎回的订单再次赎回）怎么处理？
>
> **第四维度：数据一致性 —— 覆盖跨系统校验**
>
> 金融系统最核心的。每个操作做完，查数据库确认写入正确：用户资产表、财务流水、对账记录三方的数据必须一致。我一般接口断言里直接加数据库校验，确保每一步的落库数据是对的。

**面试官考察点：**
- 有没有结构化的用例设计思维（不是零散地列场景）
- 能不能根据不同业务特点调整测试重点（金融重数据一致性、ERP 重审批流）
- 能不能把方法论落地到实际项目

**避坑提示：**
- ❌ 不要零散地说"先测正常、再测异常"——太笼统
- ✅ 要用框架把维度定清楚（场景/输入/异常/数据），面试官一听就知道你有结构化思维
- ❌ 不要只讲方法论不讲案例——面试官会觉得你在背课本
- ✅ 每个维度都套一个自己的项目案例（余币宝/永续合约/Odoo ERP）
...
> 接口测试用例我会从 6 个维度来设计：
>
> **1. 正常场景：** 覆盖 Happy Path，正常请求返回 200 和正确数据
>
> **2. 异常参数：** 必填缺失、类型错误（传 string 代替 int）、参数越界（超过最大长度/最小值）
>
> **3. 业务异常：** 重复提交、状态冲突（已审核的单子再审核）、依赖条件不满足（余额不足、库存不足）
>
> **4. 鉴权与权限：** 未登录、token 过期、无权限用户操作、越权访问（A 用户查 B 用户数据）
>
> **5. 幂等性：** 同一个请求重复发送多次，结果必须一致（下单、转账这类操作最核心）
>
> **6. 数据边界：** 大数据量返回、分页边界、排序字段测试、精度校验

**实战案例话术：**

> 在 Gate.io 测试永续合约开仓接口，除了正常开仓/平仓，我重点测了：保证金不足时是否明确拒绝、仓位上限时是否提示、同一用户并发开仓是否出现超仓、连续两次开仓指令是否被幂等拦截。这些才是线上真正容易出问题的场景。

### 8.3 测试分层策略（接口 vs UI vs AI Agent）

**面试话术（60 秒）：**

> 我的分层原则是：**接口自动化覆盖业务逻辑，UI 自动化覆盖用户交互，AI agent 覆盖探索性测试。**
>
> - **接口自动化（占 70% 用例量）：** 覆盖业务规则、数据校验、异常场景。快且稳，几分钟跑完几百条。
>
> - **UI 自动化（占 20% 用例量）：** 只覆盖核心用户流程——登录、申购、赎回等关键链路。UI 自动化维护成本高，只做接口测不到的部分。
>
> - **AI agent（占 10% 用例量）：** 做探索性测试和新功能验证。不稳定是常态，核心业务还是 Selenium 兜底。
>
> 比如在 Gate.io 测余币宝：申购/赎回业务逻辑用接口自动化 5 分钟跑完 200 条用例；UI 只覆盖"用户从首页进入 → 选择产品 → 输入金额 → 确认申购"这个端到端流程，8 条用例就够了；AI agent 用来随机操作新上线的活动页面，检查有没有异常状态。

### 8.4 敏捷项目中如何保证测试覆盖率（追问高频）

**面试话术（60 秒）：**

> 在敏捷迭代中，我主要用三个手段保证覆盖率：
>
> **1. 测试左移：** 需求评审阶段就介入，输出测试要点和风险点，避免等到提测才发现设计漏洞。
>
> **2. 分层自动化基线：** 接口自动化建一个"回归基线"——每次迭代核心用例全量跑，保证老功能不受影响。新功能用探索性测试覆盖。
>
> **3. 线上监控补位：** 线上加关键业务监控（核心链路成功率、接口耗时、资金对账差异报警），覆盖测试环境覆盖不到的场景——并发高峰、数据量膨胀、第三方超时等。
